...
def update_bitrate():
    try:
        ...
        if len(G.noises) > 1:
            currentAverageNoise = sum(G.noises) / len(G.noises)

            noiseDifference = max(currentAverageNoise - G.lastAverageNoise, 0)
            
            G.lastAverageNoise = currentAverageNoise

            # print("\nAverage Noises: \n%s" % G.lastAverageNoise)
            # print(currentAverageNoise)
            # print("Noise Difference: %s\n" % noiseDifference)
        
        G.noises = []

        # dampening function
        # checkitout: desmos.com/calculator/ncoa2xv1yl
        if noiseDifference > 0:
            bitrate = ( (-G.maxBitrate / pow(G.maxDifference, G.dampening)) * pow(noiseDifference, G.dampening) ) + G.maxBitrate
        else:
            bitrate = G.maxBitrate
...
def script_description():
  return """<center><h2>Accurately Shitty Streams!! (Peaks)</h2></center>
...
def script_properties():
    ...
    obs.obs_properties_add_int              (props, "max_difference", "Maximum Jump in Volume (in dB):", 1, 99, 10)
    obs.obs_properties_add_int              (props, "max_bitrate", "Maximum Bitrate (in kbps):", 100, 99999, 100)
    obs.obs_properties_add_float_slider     (props, "damp_scale", "Dampening:", 1, 3, 0.01)
...
def script_defaults(settings):
    obs.obs_data_set_default_int    (settings, "max_difference", 25)
    obs.obs_data_set_default_int    (settings, "max_bitrate", 5000)
    obs.obs_data_set_default_double (settings, "damp_scale", 1.6)
...
def script_update(settings):
    ...
    G.settings      = settings
    G.maxDifference = obs.obs_data_get_int(settings, "max_difference")
    G.maxBitrate    = obs.obs_data_get_int(settings, "max_bitrate")
    G.dampening     = obs.obs_data_get_double(settings, "damp_scale")
